...
import Combinations (combine, combinations, slice, groupPermutations)
...
fcomb :: String -> IO ()
fcomb = putStr . unlines . map unwords . combine . map words . lines

lcomb :: [String] -> IO ()
lcomb = putStrLn . unwords . combine
...
lgroup = putStr . unlines . map unwords . fst . unzip . map unzip . groupPermutations enumSum . combine -- should indicate how many elements on each line
